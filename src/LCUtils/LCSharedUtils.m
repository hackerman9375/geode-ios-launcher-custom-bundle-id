...
+ (NSURL*)appLockPath {
    ...
    dispatch_once(&once, ^{
        infoPath = [[LCSharedUtils appGroupPath] URLByAppendingPathComponent:@"Geode/appLock.plist"];
    });
    return infoPath;
}
...
+ (NSURL*)containerLockPath {
    ...
    dispatch_once(&once, ^{
        infoPath = [[LCSharedUtils appGroupPath] URLByAppendingPathComponent:@"Geode/containerLock.plist"];
    });
    return infoPath;
}
